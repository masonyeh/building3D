    ...
    if (_obj.show == null || typeof (_obj.show) == 'undefined' || _obj.show) {
        ...
        switch (_obj.objType) {
            ...
            case 'cloneObj':
                ...
                break;
            case 'smoke':
                _tempObj = _this.createSmokeDetector(_this, _obj);
                _this.addObject(_tempObj);
                break;
          }
    }
...
//创建烟感
msj3D.prototype.createSmokeDetector = function (_this,_obj) {
    if (_this == null) {
        _this = this;
    }
    var _length = _obj.length || 2;//默认1000
    var _width = _obj.width || 2;
    var _height = _obj.height || 4;
    var _x = _obj.x || 0, _y = _obj.y || 0, _z = _obj.z || 0;
    var skinColor = _obj.style.skinColor || 0x98750f;
    var smokeGeometry = new THREE.CylinderGeometry(_length, _width,  _height, 18, 3);

    //六面颜色
    for (var i = 0; i < smokeGeometry.faces.length; i += 2) {
        var hex = skinColor || Math.random() * 0x531844;
        smokeGeometry.faces[i].color.setHex(hex);
        smokeGeometry.faces[i + 1].color.setHex(hex);
    }
    //六面纹理
    var skin_up_obj = {
    ...
    var skin_center_obj = skin_center_obj
    var skin_opacity = 1;
    if (_obj.style != null && typeof (_obj.style) != 'undefined'
        ...
        //上
        skin_up_obj = _this.createSkinOptionOnj(_this, _length, _width, _obj.style.skin.skin_up, smokeGeometry, 4);
        //下
        skin_down_obj = _this.createSkinOptionOnj(_this, _length, _width, _obj.style.skin.skin_down, smokeGeometry, 6);
        //中间
        skin_center_obj = _this.createSkinOptionOnj(_this, _length, _width, _obj.style.skin.skin_fore, smokeGeometry, 0);
    }
    var SmokeMaterialArray = [];
    SmokeMaterialArray.push(new THREE.MeshLambertMaterial(skin_up_obj));
    SmokeMaterialArray.push(new THREE.MeshLambertMaterial(skin_down_obj));
    SmokeMaterialArray.push(new THREE.MeshLambertMaterial(skin_center_obj));
    var smokeMaterials = new THREE.MeshFaceMaterial(SmokeMaterialArray);
    SmokeDetector = new THREE.Mesh(smokeGeometry, smokeMaterials);
    SmokeDetector.castShadow = true;
    SmokeDetector.receiveShadow = true;
    SmokeDetector.uuid = _obj.uuid;
    SmokeDetector.name = _obj.name;
    SmokeDetector.position.set(_x, _y, _z);
    if (_obj.rotation != null && typeof (_obj.rotation) != 'undefined' && _obj.rotation.length > 0) {
        _obj.rotation.forEach(function (v, i) {
            switch (v.direction) {
                case 'x':
                    SmokeDetector.rotateX(v.degree);
                    break;
                case 'y':
                    SmokeDetector.rotateY(v.degree);
                    break;
                case 'z':
                    SmokeDetector.rotateZ(v.degree);
                    break;
                case 'arb':
                    SmokeDetector.rotateOnAxis(new THREE.Vector3(v.degree[0], v.degree[1], v.degree[2]), v.degree[3]);
                    break;
            }
        })
    }
